...
router.put('/:id', function (req, res, next) {
  ...
  models.actor.update(req.body, { where: { actor_id: actorId } })
    ...
    .catch(err => {
      res.status(400)
      res.send("there was a problem updating actor, please check actor info")
    })
})
...
router.delete('/:id', function (req, res, next) {
  ...
  models.actor.destroy({
    ...
    .catch(err => {
      res.status(400)
      res.send("There was a problem deleting the actor, please insure you are specifying the correct id")
    })
})
